{{
    config(
        ...
        file_format='parquet',
        pre_hook={
            'sql': 'create or replace function rariable_exchangestatev1_getcompletedkey_calldecodeudf as "io.iftech.sparkudf.hive.Rariable_ExchangeStateV1_getCompletedKey_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.0.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x30786463"
    and selector_hash = abs(hash("0x30786463")) % 10
