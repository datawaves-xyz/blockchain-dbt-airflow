{{
    config(
        ...
        file_format='parquet',
        pre_hook={
            'sql': 'create or replace function looksrare_looksrareexchange_updateroyaltyfeemanager_calldecodeudf as "io.iftech.sparkudf.hive.Looksrare_LooksRareExchange_updateRoyaltyFeeManager_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.0.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x30786335"
    and selector_hash = abs(hash("0x30786335")) % 10
