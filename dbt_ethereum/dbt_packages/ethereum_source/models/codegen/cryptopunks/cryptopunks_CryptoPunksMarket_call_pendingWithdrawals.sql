{{
    config(
        ...
        file_format='parquet',
        pre_hook={
            'sql': 'create or replace function cryptopunks_cryptopunksmarket_pendingwithdrawals_calldecodeudf as "io.iftech.sparkudf.hive.Cryptopunks_CryptoPunksMarket_pendingWithdrawals_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.0.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x30786633"
    and selector_hash = abs(hash("0x30786633")) % 10
