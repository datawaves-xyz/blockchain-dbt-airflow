{{
    config(
        ...
        file_format='parquet',
        pre_hook={
            'sql': 'create or replace function superrare_superrare_setsaleprice_calldecodeudf as "io.iftech.sparkudf.hive.Superrare_SuperRare_setSalePrice_CallDecodeUDF" using jar "s3a://blockchain-dbt/dist/jars/blockchain-dbt-udf-0.1.0.jar";'
        }
    )
}}
...
with base as (
    ...
    and selector = "0x30783035"
    and selector_hash = abs(hash("0x30783035")) % 10
